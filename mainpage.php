...
else if((!empty($sloc) && !empty($dloc))){ 
    $query1 = "select username from matches where start_location='$sloc' and destination_location = '$dloc' and username != '".$c_user."'";
    $result1 = mysqli_query($conn, $query1);
}
else if(((!empty($loc)) && (!empty($int1) && !empty($int2)))){
    $query1 = "select username from matches where location='$loc' and interest1 = '$int1' and interest2 = '$int2' and username != '".$c_user."'";
    $result1 = mysqli_query($conn, $query1);
}
...
if(!empty($query1) && $flag==0 && mysqli_num_rows($result1) > 0){
    $count = 0;
...
//                 $count = 0;
//                while($user = mysqli_fetch_assoc($result1)){
//                 $query1_2 = "select firstname, mail, phonenumber from user_details where username = '".$user['username']."'";
//                 $result1_2 = mysqli_query($conn, $query1_2);
//                 $type = "travel partner";
//                 if($count == 0){
//                     $response = "";
...
//                 while($r = mysqli_fetch_assoc($result1_2)){
//                     $resulttable="resulttable";
//                   $response = $response."<tr class=".$resulttable."><td>".$count."</td><td>".$type."</td><td>".$r['lastname']."</td><td>".$r['mail']."</td><td>".$r['phonenumber']."</td></tr>"; 
//                 } 
